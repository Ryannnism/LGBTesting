...
RDS SQL Server Express does **not** create a named database. After apply, SSH to the VM (or use any SQL client allowed by the security group) and:

```sql
CREATE DATABASE LGBApp;
```

Then put `terraform output -raw connection_string` into `/etc/lgbapp/lgbapp.env`. The API's `Database.Migrate()` creates tables on first boot.
